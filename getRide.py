...
def getUpcomingRides():
	...
	formFields = (
	   ...
	   # Fields of interest: search criteria
		...
		(r'ctl00$mainPanel$MobMainPanel1$MobRealTime1$txtStop', ''),
		(r'ctl00$mainPanel$MobMainPanel1$MobRealTime1$btnGO', 'GO')
	)

	for route in routes:
		formFields = formFields[0], formFields[1], (r'ctl00$mainPanel$MobMainPanel1$MobRealTime1$txtStop', route), formFields[3]
		# Encode fields
...
routes = (2057, 1767, 1723, 2060)
getUpcomingRides()
fin = open('2057.html', 'r')
fin2 = open('a.html', 'w')
soup = BeautifulSoup(fin)
found = soup.find_all(id="ctl00_mainPanel_MobMainPanel1_MobRealTime1_gvSearchResult")
print(found)
fin2.write(str(found))
fin.close()
